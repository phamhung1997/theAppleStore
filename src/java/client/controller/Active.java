...
public class Active extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            String code = request.getParameter("txtActiveCode");
            HttpSession session = request.getSession(true);
            Account account = (Account)session.getAttribute("currentLoginAccount");
            String email = account.getEmail();
            
            AccountService accountService = new AccountService();
            if(accountService.activeCode(email, code)){
                session.removeAttribute("currentLoginAccount");
                response.sendRedirect("login.jsp");
            } else {
                request.setAttribute("message", IMessageConfig.ACTIVE_CODE_NOT_MATCH);
